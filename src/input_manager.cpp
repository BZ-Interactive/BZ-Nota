...
bool InputManager::handle_event(Event event, Editor& editor, volatile sig_atomic_t& ctrl_c_pressed) {
    // Currently ignore all mouse events
    if (event.is_mouse()) return true;

    // Reset status bar variables on every event
    editor.reset_status();
    ...
    if (handle_navigation_sequences(event.input(), editor, editor.is_debug_mode())) return true;

    // Handle standard keys
    if (handle_standard_keys(event, editor)) return true;

    // Handle regular text input
    if (handle_text_input(event, editor)) return true;

    return false;
}

bool InputManager::handle_ctrl_keys(unsigned char ch, Editor& editor) {
    switch (ch) {
...
bool InputManager::handle_standard_keys(Event event, Editor& editor) {
    // Arrow keys
...
bool InputManager::handle_text_input(Event event, Editor& editor) {
