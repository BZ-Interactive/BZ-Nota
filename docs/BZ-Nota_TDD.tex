...
\begin{table}[H]
	...
	\begin{tabular}{|l|l|}
		...
		\hline
		\textbf{Programming Language} & C++23                                   \\
		\hline
...
\begin{table}[H]
	...
	\begin{tabular}{|c|c|c|p{8cm}|}
		...
		\hline
		1.2              & \date{01.03.2026} & Barkın Zorlu    & Organization Edits to TDD                                                 \\
		\hline
		1.3              & \date{02.03.2026} & Barkın Zorlu    & Update to v0.7.0 - ALT formatting shortcuts                               \\
		\hline
		1.4              & \date{03.03.2026} & Barkın Zorlu    & Remove explicit Kitty protocol, use Alt+key for formatting                \\
		\hline
		1.5              & \date{03.03.2026} & Barkın Zorlu    & Add dark/light mode toggle via F8 (dark is default)                       \\
		\hline
		1.6              & \date{03.03.2026} & Barkın Zorlu    & Document Editor Modes and F7 toggle (BASIC/FANCY/CODE/DOCUMENT)           \\
		\hline
		1.7              & \date{03.03.2026} & Barkın Zorlu    & Update C++ version to 23 to match current standard                        \\
		\hline
		1.8              & \date{26.03.2026} & Barkın Zorlu    & Add Config Manager for persistent settings (theme, future customizations) \\
		\hline
		1.9              & \date{30.03.2026} & Barkın Zorlu    & Add Privilege Elevation support (sudo, doas, pkexec) \\
		\hline
	\end{tabular}
\end{table}
...
BZ-Nota is a modern terminal user interface (TUI) text editor built entirely in C++23. It combines the intuitive editing experience of Micro with a sophisticated, btop-style dashboard interface to provide users with an enhanced terminal editing experience.
...
\begin{table}[H]
	...
	\begin{tabular}{|p{5cm}|p{8cm}|}
		...
		\hline
		\textbf{File Manager}      & Handles file I/O operations, file state management, and privilege elevation \\
		\hline
		...
		\hline
		\textbf{Config Manager}    & Persists user preferences (theme, future customizations)                     \\
		\hline
	\end{tabular}
\end{table}
...
\begin{itemize}
	...
	\item Handles terminal escape sequences for enhanced key detection
	...
	\item Dark mode is the default theme (dark background, light foreground)
	\item Theme toggle via F8 provides instant switching between dark and light modes
	\item Theme preference is persisted and restored on next launch via ConfigManager
\end{itemize}
...
\begin{itemize}
	...
	\item Enhanced key event discrimination for modifier combinations
\end{itemize}
...
// Appearance
F7			   => Toggle Editor mode (switches between BASIC, FANCY, CODE, DOCUMENT modes)
F8             => Toggle dark/light mode (instant switch, dark is default)
...
\subsection{Privilege Confirmation Mode}

When a file save fails due to insufficient permissions (EACCES error), the Input Manager enters a special privilege confirmation state. In this mode:

\begin{itemize}
	\item The UI displays a confirmation prompt:\\
	\texttt{Permission denied! Save with [sudo/doas/pkexec]? (y/n)}
	\item Normal input handling is suspended
	\item Only \texttt{y}, \texttt{Y}, \texttt{n}, \texttt{N}, and \texttt{Escape} keys are processed
	\item User confirmation triggers privilege elevation via FileManager
	\item User rejection cancels the save operation and returns to normal editing
	\item Escape key cancels the operation
\end{itemize}

The InputManager maintains state with:
\begin{itemize}
	\item \texttt{is\_privilege\_confirm}: Boolean flag indicating privilege confirmation mode is active
	\item \texttt{handle\_privilege\_confirm\_input()}: Dedicated handler for y/n confirmation
\end{itemize}
...
BZ-Nota supports multiple editing modes to suit different use cases. Switch between modes using \textbf{F7} (cycles: BASIC $\rightarrow$ FANCY $\rightarrow$ CODE $\rightarrow$ DOCUMENT $\rightarrow$ BASIC).

\subsection{BASIC Mode}

Plain text editing with no formatting support. Markdown markers like \texttt{**text**} or \texttt{*italic*} are displayed literally as raw text without any styling applied. This is meant to be used when you want exact byte-for-byte content. Formatting shortcuts (Alt+B/I/U/T) are disabled in this mode.

\subsection{FANCY Mode (Default)}

Document editing with Markdown formatting support. Text wrapped in formatting markers is rendered with appropriate styling in the editor UI:
\begin{itemize}
	\item \texttt{**bold**} $\rightarrow$ displayed as \textbf{bold}
	\item \texttt{*italic*} $\rightarrow$ displayed as \textit{italic}
	\item \texttt{<u>underline</u>} $\rightarrow$ displayed as \underline{underline}
	\item \texttt{\textasciitilde\textasciitilde strikethrough\textasciitilde\textasciitilde} $\rightarrow$ displayed as strikethrough
\end{itemize}
Users can toggle formatting with \textbf{Alt+B/I/U/T} while typing or apply to selections. Multiple formats can be stacked. This is the default mode suitable for note-taking and documentation.

\subsection{CODE Mode}

Syntax highlighting for programming languages with color-coded display of keywords, strings, comments, and other language constructs. Markdown formatting markers are displayed literally (like BASIC mode). \textbf{Currently in development - falls back to BASIC behavior.}

\subsection{DOCUMENT Mode}

Enhanced FANCY mode with support for dictionary checking, grammar suggestions, and document-specific features. \textbf{Currently in development - falls back to FANCY behavior.}

\medskip
\textit{Note: In the current implementation, F7 toggles between BASIC and FANCY modes only. CODE and DOCUMENT modes may not be implemented if deemed out of scope for the initial release.}
...
\begin{itemize}
	...
	\item Privilege elevation for permission-denied saves
\end{itemize}

\subsection{Privilege Elevation Support}

BZ-Nota supports automatic privilege elevation for files that require elevated permissions to write. The system automatically detects and uses the first available privilege elevation tool in the following priority:

\begin{enumerate}
	\item \textbf{sudo} - Most common, supports non-interactive cached credentials via \texttt{sudo -n}
	\item \textbf{doas} - OpenBSD/Linux alternative with stricter security model (interactive only)
	\item \textbf{pkexec} - PolicyKit-based elevation for desktop environments (interactive only)
\end{enumerate}

\subsubsection{Privilege Save Workflow}

When a save operation fails with permission denied (EACCES):

\begin{enumerate}
	\item \textbf{Cache Check}: System attempts to verify if elevated privileges are already cached (sudo only)
	\item \textbf{Non-Interactive Path}: If privileges are cached, saves silently via \texttt{sudo -n tee}
	\item \textbf{Interactive Path}: If no cached privileges, prompts user with confirmation and invokes the privilege tool
	\item \textbf{User Prompt}: UI displays "Permission denied! Save with [tool]? (y/n)" and awaits confirmation
	\item \textbf{Execution}: On confirmation, writes buffer to temporary file in \texttt{/tmp/bznota\_priv\_[PID].tmp} and uses the selected tool to copy to target location
	\item \textbf{Cleanup}: Temporary file is securely deleted after operation completes
\end{enumerate}

\subsubsection{Implementation Details}

\begin{itemize}
	\item \textbf{Tool Detection}: \texttt{get\_privilege\_tool()} uses shell \texttt{command -v} to detect available tools at runtime
	\item \textbf{Cache Detection}: \texttt{privilege\_is\_cached()} checks if sudo credentials are valid (sudo only)
	\item \textbf{Temp Files}: Uses process ID in filename to avoid conflicts in multi-user environments
	\item \textbf{Shell Quoting}: Paths are properly escaped to handle special characters and spaces
	\item \textbf{Terminal Control}: Interactive mode clears terminal and displays informative prompt
\end{itemize}

\subsubsection{Security Considerations}

\begin{itemize}
	\item Temporary files are created in \texttt{/tmp} with restrictive permissions (0600)
	\item Files are cleaned up immediately after privilege operation completes
	\item Non-interactive \texttt{sudo -n} prevents password prompts from breaking the UI
	\item All paths are properly shell-quoted to prevent injection attacks
	\item The system respects existing privilege cache (does not prompt if already authorized)
\end{itemize}

\section{Config Manager Component}

\subsection{Responsibility}

The Config Manager handles persistent user preferences:

\begin{itemize}
	\item Saves and loads user configuration from config file
	\item Manages theme preference (light/dark mode)
	\item Future: colour customization options
	\item Future: limited control customizations
\end{itemize}

\subsection{Design Details}

\begin{itemize}
	\item Configuration stored in \texttt{\textasciitilde/.config/bznota/config.toml }
	\item TOML-based configuration format
	\item Currently stores last used theme preference
	\item Designed to be extensible for future customization options
	\item Graceful handling of missing or corrupted config files (defaults to dark theme)
\end{itemize}
...
{
  "theme": "dark" | "light",
  "colours": { ... },    // Future: custom colour palette
  "controls": { ... }    // Future: key binding customizations
}
\end{lstlisting}
...
public:
    ...
    FileOperationResult save_file_with_privilege(
...
\begin{table}[H]
	...
	\begin{tabular}{|l|l|l|}
		...
		\hline
		Language           & C++23               & High performance, modern features \\
		\hline
		Build System       & CMake 3.10+         & Cross-platform build automation   \\
		\hline
		UI Framework       & FTXUI               & Terminal UI rendering             \\
		\hline
		Config Parser      & toml++              & Configuration parsing and serialization\\
		\hline
...
\par
The installation prefers environments' default directory listings.
...
\begin{table}[H]
	...
	\begin{tabular}{|l|p{8cm}|}
		...
		\hline
		config\_manager.hpp/cpp     & Persistent user preferences (theme, future customizations)                          \\
		\hline
	\end{tabular}
\end{table}
...
\begin{figure}[H]
	...
	\begin{tikzpicture}[->, very thick, node distance=2.2cm]
		...
		\node (config) [draw, rectangle, fill=blue!20, right=3cm of shared] {ConfigManager};
...
\begin{table}[H]
	...
	\begin{tabular}{|p{4cm}|p{7cm}|}
		...
		\hline
		Privilege save not working  & Verify sudo/doas/pkexec installed: \texttt{which sudo doas pkexec}      \\
		\hline
		Privilege prompt not appearing & Check terminal can execute shell commands; check /tmp is writable      \\
		\hline
		Sudo always prompts password & Use \texttt{sudo -v} to refresh credentials; configure sudoers for nopass \\
		\hline
	\end{tabular}
\end{table}
...
The design follows proven software engineering principles and patterns, ensuring code quality and long-term maintainability. With its foundation on robust technologies like C++23 and FTXUI, BZ-Nota is positioned for continued growth and enhancement.
...
\begin{itemize}
	...
	\item C++23 Standard: ISO/IEC 14882:2024
